...
function deletedata(id) {
    ...
    postbody.innerHTML = dataArr.map((item, ind) => {
        ...
        <div class="dateshower">
            <div class="my-4">
                <button type="button" id=${item.id} onclick=editdata(this.id) class="btn btn-light border border-dark" data-bs-toggle="modal" data-bs-target="#exampleModal2">
                     Edit Post
                 </button>
                   <button type="button" id=${item.id} onclick=deletedata(this.id) class="btn btn-light border border-dark">Delete Post</button>
                 </div>
...
function editdata(id) {
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Edit A post</h1>
      ...
      <div class="modal-footer">
        ...
        <button type="button" id="saveBlogbut" onclick=editsavepost(document.getElementById('exampleFormControlInputedit1').value,document.getElementById('exampleFormControlTextareaedit2').value,${id}) data-bs-dismiss="modal" class="btn btn-light border border-dark">Save Post</button>
        <button type="button" class="btn btn-light border border-dark" onclick=deletedata(id) data-bs-dismiss="modal">Delete Post</button>
      </div>
    </div>
    </div> `
...
saveBlogbut.addEventListener("click", function () {
    ...
    postbody.innerHTML = dataArr.map((item, ind) => {
        ...
        <div class="dateshower">
            <div class="my-4">
                <button type="button" id=${item.id} onclick=editdata(this.id) class="btn btn-light border border-dark  my-2" data-bs-toggle="modal" data-bs-target="#exampleModal2">
                     Edit Post
                 </button>
                   <button type="button" id=${item.id} onclick=deletedata(this.id) class="btn btn-light border border-dark my-2">Delete Post</button>
                 </div>
